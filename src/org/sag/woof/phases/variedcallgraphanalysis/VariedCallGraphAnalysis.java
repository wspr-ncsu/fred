package org.sag.woof.phases.variedcallgraphanalysis;
...
public class VariedCallGraphAnalysis {
	...
	public boolean run() {
		...
		if(dumpnative) {
			Path dumpFile = dataAccessor.getConfig().getFilePath("debug_woof-eps-to-native-methods-dump-file");
			Path dumpFile2 = dataAccessor.getConfig().getFilePath("debug_woof-native-methods-from-eps-dump-file");
	...
	private boolean dumpNative(Path outputPath, Path outputPath2) {
		...
		final Map<EntryPoint,Set<SootMethod>> epsToNativeMethods = new HashMap<>();
		boolean successOuter = true;
						...
						try {
							...
							if(!nativeMethods.isEmpty()) {
								...
								synchronized(epsToNativeMethods) {
									epsToNativeMethods.put(ep, nativeMethods);
								}
							}	
		...
		if(!epsToNativeMethods.isEmpty()) {
			Map<EntryPoint,Set<SootMethod>> epsToNativeMethodsSorted = SortingMethods.sortMapKeyAscending(epsToNativeMethods);
			Set<SootMethod> nativeMethodsUniq = new HashSet<>();
			for(Set<SootMethod> s : epsToNativeMethodsSorted.values()) {
				nativeMethodsUniq.addAll(s);
			}
			nativeMethodsUniq = SortingMethods.sortSet(nativeMethodsUniq,SootSort.smComp);
			try {
				try(PrintStream ps = new PrintStreamUnixEOL(Files.newOutputStream(outputPath))) {
					for(EntryPoint ep : epsToNativeMethodsSorted.keySet()) {
						ps.println(ep);
						for(SootMethod sm : epsToNativeMethodsSorted.get(ep)) {
							ps.println("  " + sm);
						}
					}
				}
	...
	public static void main(String[] args) throws IOException {
		BufferedReader br = Files.newBufferedReader(FileHelpers.getPath("C:\\CS\\Documents\\Work\\Research\\woof\\aosp-10.0.0\\debug\\2020-05-30_11-59-26\\woof_eps_to_native_methods_dump.txt"));
		String ln;
		Map<String,Set<String>> epsToNative = new HashMap<>();
		Set<String> curNativeMethods = null;
		while((ln = br.readLine()) != null) {
			if(!ln.startsWith("{")) {
				curNativeMethods.add(ln.trim());
			} else {
				curNativeMethods = new HashSet<>();
				epsToNative.put(ln.trim(), curNativeMethods);
			}
		}
		br.close();
		
		br = Files.newBufferedReader(FileHelpers.getPath("C:\\CS\\Documents\\Work\\Research\\woof\\aosp-10.0.0\\debug\\2020-05-30_11-04-26\\eps_to_native_methods_dump.txt"));
		curNativeMethods = null;
		while((ln = br.readLine()) != null) {
			if(!ln.startsWith("{")) {
				curNativeMethods.add(ln.trim());
			} else {
				curNativeMethods = epsToNative.get(ln.trim());
				if(curNativeMethods == null) {
					curNativeMethods = new HashSet<>();
					epsToNative.put(ln.trim(),curNativeMethods);
				}
			}
		...
		for(String ep : epsToNative.keySet()) {
			epsToNative.put(ep, SortingMethods.sortSet(epsToNative.get(ep),SootSort.smStringComp));
		}
		epsToNative = SortingMethods.sortMapKeyAscending(epsToNative);
		
		try(PrintStream ps = new PrintStreamUnixEOL(Files.newOutputStream(FileHelpers.getPath("C:\\CS\\Documents\\Work\\Research\\woof\\aosp-10.0.0\\debug\\test.txt")))) {
			for(String ep : epsToNative.keySet()) {
				ps.println(ep);
				for(String sm : epsToNative.get(ep)) {
					ps.println("  " + sm);
				}
			}
		}
